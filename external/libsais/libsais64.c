...
static sa_sint_t libsais64_main_32s_recursion(sa_sint_t * RESTRICT T, sa_sint_t * RESTRICT SA, sa_sint_t n, sa_sint_t k, sa_sint_t fs, sa_sint_t threads, LIBSAIS_THREAD_STATE * RESTRICT thread_state, sa_sint_t * RESTRICT local_buffer)
{
    ...
    // adjusted by Jan Zumbrink
    //
    // the following section caused the corruption of part of the given T array
    /*if (n <= INT32_MAX)
    {
...
static void libsais64_compute_plcp_int(const sa_sint_t* RESTRICT T, sa_sint_t * RESTRICT PLCP, fast_sint_t n, fast_sint_t omp_block_start, fast_sint_t omp_block_size)
{
    const fast_sint_t prefetch_distance = 32;
...
static void libsais64_compute_plcp_int_omp(const sa_sint_t* RESTRICT T, sa_sint_t * RESTRICT PLCP, sa_sint_t n, sa_sint_t threads)
{
#if defined(LIBSAIS_OPENMP)
    #pragma omp parallel num_threads(threads) if(threads > 1 && n >= 65536)
#endif
    {
        ...
        libsais64_compute_plcp_int(T, PLCP, n, omp_block_start, omp_block_size);
    }
}
...
int64_t libsais64_plcp_int(const int64_t* T, const int64_t* SA, int64_t* PLCP, int64_t n) {
    if ((T == NULL) || (SA == NULL) || (PLCP == NULL) || (n < 0))
    {
        return -1;
    }
    ...
    libsais64_compute_phi_omp(SA, PLCP, n, 1);
    libsais64_compute_plcp_int_omp(T, PLCP, n, 1);
